...
with st.expander("주의사항 및 면책 조항"):
    ...
    3. **연금 수령**: 연금 수령은 **매년 초**에 이루어진다고 가정합니다. 실제로는 연, 월, 또는 분기 단위로 수령 주기를 설정할 수 있습니다. 또한, 수령 주기를 연간으로 가정하였으므로, 다른 수령 주기를 선택할 경우 받는 금액이 실제와는 달라질 수 있습니다.
    4. **수익률 및 물가**: 입력된 값이 매년 일정하게 유지된다고 가정한 결과를 보여주므로, 실제 투자 수익과 큰 괴리가 발생할 수 있습니다. 투자는 원금 손실의 위험이 있음을 참고하세요.
    5. **면책 조항**: 이 계산기는 사용자의 편의를 위한 예상치 제공을 목적으로 하며, 어떠한 경우에도 재정적 조언이나 법적 자문으로 간주될 수 없습니다. 계산 결과는 입력된 가정과 현재 세법을 기반으로 하지만, 오류가 있을 수 있으므로 실제 결과와 차이가 발생할 수 있습니다. 이용자는 이 계산기 결과에만 의존하여 투자 또는 재정 결정을 내리지 않아야 하며, 모든 재정적 결정에 대한 최종 책임은 사용자 본인에게 있습니다.
    """)
