...
def display_initial_summary(inputs: UserInput, total_at_retirement, simulation_df, total_tax_credit):
    ...
    col3.metric("월간 수령액 (세후)", f"{monthly_take_home:,.0f} 원", help="연간 수령액을 12로 나눈 값입니다.") # 새로운 월간 수령액 열에 도움말 추가
...
with st.sidebar:
    ...
    if st.button("결과 확인하기", type="primary"):
        ...
        # Removed the current_age_actual < start_age validation
        ...
        if not simulation_df.empty:
            ...
            if choice_df.empty:
                ...
                st.info(
                    ...
                    "이 선택권은 과거 신고 방식과 무관하게 매년 부여되므로, "
                    "연간 소득이 기준을 초과하는 해마다 다시 한 번 유불리를 따져 과세 방식을 결정하시면 됩니다."
                )
