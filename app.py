...
def display_initial_summary(inputs: UserInput, total_at_retirement, simulation_df, total_tax_credit):
    ...
    """
    st.header("📈 예상 결과 요약 (직접 입력 수익률 기준)")
    # 첫 해 연간 실수령액 계산
...
def display_asset_visuals(inputs: UserInput, total_at_retirement: float, total_principal: float, total_non_deductible_paid: float):
    ...
    with col2:
        ...
        fig_pie = px.pie(pie_data, values='금액', names='항목', hole=.3, color_discrete_sequence=px.colors.sequential.Blues_r)
...
def initialize_session():
    ...
    st.session_state.pre_retirement_return = 6.0 # 기본 수익률 (중립형)
    st.session_state.post_retirement_return = 4.0 # 기본 수익률 (중립형)
...
if st.session_state.get('calculated', False):
    ...
    if total_at_retirement > 0:
        ...
        if not simulation_df.empty:
            ...
            st.markdown("---") # 구분선 추가
            st.header("📜 연금 인출 상세 시뮬레이션 (직접 입력 기준)")
